...
def performSentimentAnalysis(toBeProcessed):
    classifier = None
    if path.exists("../models/sentimentClassifier.pk"):
        print("exists")
        v = open("../models/sentimentClassifier.pk", 'rb')
        classifier = pickle.load(v)
    else:
        ...
        with open('../models/sentimentClassifier.pk', 'wb') as fin:
